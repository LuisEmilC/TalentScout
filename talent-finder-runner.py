...
UA='TalentScout-Talent-Finder/7.5 (https://github.com/LuisEmilC/TalentScout; public scouting project)'
SPARQL='https://query.wikidata.org/sparql?format=json&query='; SPARQL_POST='https://query.wikidata.org/sparql'
WD='https://www.wikidata.org/w/api.php?action=wbgetentities&ids={}&format=json&props=claims|labels'; SS='https://www.sofascore.com/api/v1/search/all?q={}'; TM='https://www.transfermarkt.com/schnellsuche/ergebnis/schnellsuche?query={}'
...
SELECT DISTINCT ?person ?personLabel ?birth ?club ?clubLabel ?countryLabel WHERE {{
 ...
 SERVICE wikibase:label {{ bd:serviceParam wikibase:language "en". }}
    ...
    for r in data.get('results',{}).get('bindings',[]):
        ...
        counts[country]=counts.get(country,0)+1;out.append({'qid':r['person']['value'].rsplit('/',1)[-1],'name':r['personLabel']['value'],'clubQid':r['club']['value'].rsplit('/',1)[-1],'club':club,'country':country})
    return out
...
def verify(c,now):
    ...
    try:
        ent=get(WD.format(c['qid']),20).get('entities',{}).get(c['qid'],{});cl=ent.get('claims',{});label=ent.get('labels',{}).get('en',{}).get('value','').strip();a=age(next(iter(vals(cl,'P569')),None),now.date());current=qids(cl,'P54');nat_ids=qids(cl,'P27');pos_ids=qids(cl,'P413');nationality=entity_label(nat_ids[0]) if nat_ids else None;position=entity_label(pos_ids[0]) if pos_ids else None
        ...
        try:
            p=sofa(c['name'])
            if p:sofa_ok=True;team=(p.get('team') or {}).get('name');sofa_nat=(p.get('country') or {}).get('name');sofa_pos=p.get('position');sources.add('Sofascore')
        ...
        if len(sources)<2 and not technical:reasons.append('minder dan 2 onafhankelijke bronnen')
    ...
    except Exception as e:return None,[],[],{'candidateId':c.get('qid'),'name':c.get('name'),'club':c.get('club'),'country':c.get('country'),'errorType':type(e).__name__,'error':str(e)}

def main():
    started=datetime.now(timezone.utc);max_minutes=max(1,int(os.getenv('TALENT_FINDER_RUN_MINUTES','50')));approved=[];rejected=[];errors=[];screened=0;seen=set();passes=0;index=0
    while (datetime.now(timezone.utc)-started).total_seconds()<max_minutes*60:
        passes+=1;batch=[COUNTRIES[(index+i)%len(COUNTRIES)] for i in range(4)];index=(index+4)%len(COUNTRIES)
        try:candidates=discover(batch,8)
        except Exception as e:errors.append({'stage':'discovery','countries':batch,'errorType':type(e).__name__,'error':str(e)});time.sleep(5);continue
        for c in candidates:
            if c['qid'] in seen:continue
            seen.add(c['qid']);screened+=1;p,checks,reasons,error=verify(c,datetime.now(timezone.utc))
            if error:errors.append({'stage':'candidate_verification',**error});continue
            if p and p['verification']['status']=='verified' and p['verification']['checkCount']>=5 and len(p['sources'])>=2 and not p['verification']['hardConflict']:approved.append(p)
            else:rejected.append({'id':f'candidate:{c["qid"]}','name':c['name'],'country':c['country'],'club':c['club'],'reason':' / '.join(reasons) or 'verification failed','failedControls':[n for n,ok in checks if not ok],'checksPerformed':7,'checksPassed':sum(ok for _,ok in checks),'mainControlCount':7,'supportingSources':sorted((p or {}).get('sources',[]))})
        time.sleep(2)
    by_id={p['id']:p for p in approved};approved=list(by_id.values())
    ...
    report={'runStartedAt':started.isoformat(),'runCompletedAt':completed.isoformat(),'status':'completed','runType':'continuous automated real public-source scouting run','passesCompleted':passes,'countriesScouted':COUNTRIES,'candidatesScreened':screened,'approvedPlayers':approved,'rejectedPlayers':rejected,'runnerErrors':errors,'technicalUnresolved':errors,'existingPlayersPreserved':True,'unexpectedDeletions':False,'jsonValid':True,'writeVerification':'workflow_run sync-after-talent-finder-run','verificationPolicy':'Exactly 7 main controls; 5/7, 6/7 or 7/7 may pass; 4/7 or lower is rejected; hard contradictions block; minimum 2 independent public sources; technical source failures are runnerErrors, never player rejections; missing optional statistics never block approval.'}
    out.write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n',encoding='utf-8');print(json.dumps({'report':str(out),'passes':passes,'screened':screened,'approved':len(approved),'rejected':len(rejected),'technicalErrors':len(errors)}))
