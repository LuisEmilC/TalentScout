...
UA='TalentScout-Talent-Finder/6.0 (+https://github.com/LuisEmilC/TalentScout)'
...
SELECT ?person ?personLabel ?birth ?club ?clubLabel ?countryLabel WHERE {{
 ...
 ?club wdt:P31/wdt:P279* wd:Q847017; wdt:P17 ?country.
...
def verify(c,now):
    ...
    try:
        ...
        # Exactly 7 MAIN controls. Evidence from additional sources is sub-evidence, not extra controls.
        ...
        # Seventh control: independent corroboration of current player/club from a third source or official club site.
        ...
        player={'id':f'official:wikidata:{c["qid"]}','name':c['name'],'age':a,'club':team or c['club'],'position':position,'nationality':nationality,'country':c['country'],'sources':sorted(sources),'verification':{'status':'verified','checkedAt':now.isoformat(),'checksPassed':[n for n,ok in checks if ok],'checkCount':passed,'mainControlCount':7,'controls':{n:ok for n,ok in checks}}}
